...
Public Class ucUdpChat
    ...
    Private Sub SendUdpPayload(payload As String)
        udpClient.Send(System.Text.Encoding.ASCII.GetBytes(payload), System.Text.Encoding.ASCII.GetBytes(payload).Length, txtudpip.Text, txtudpport.Text)
    End Sub
    Private Sub StartUdpReceive()
        Dim rport As Integer = txtudpportreceive.Text
        receivingUdpClient = New System.Net.Sockets.UdpClient(rport)
        NewInitialize()
        cmdudpreceive.Enabled = False
        cmdstopreceive.Enabled = True
        txtudpportreceive.Enabled = False
    End Sub
    Private Sub StopUdpReceive()
        If ThreadReceive IsNot Nothing Then ThreadReceive.Abort()
        If receivingUdpClient IsNot Nothing Then receivingUdpClient.Close()
        cmdstopreceive.Enabled = False
    ...
    Private Sub cmdudpsend_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdudpsend.Click
        ...
        SendUdpPayload(txtudpdata.Text)
    End Sub
